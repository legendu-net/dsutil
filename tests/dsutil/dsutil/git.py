...
def _git_status(gcmd) -> List[bytes]:
    proc = sp.run(
        gcmd + ['status'], check=True, stdout=sp.PIPE, stderr=sp.STDOUT
    )
...
def _git_current_branch(gcmd) -> bytes:
    proc = sp.run(
        gcmd + ['branch'], check=True, stdout=sp.PIPE, stderr=sp.STDOUT
    )
    lines = [
        line.strip() for line in proc.stdout.splitlines() if line.strip() != b''
    ]
...
def _git_remotes(gcmd) -> Set[bytes]:
    proc = sp.run(
        gcmd + ['remote', '-v'], check=True, stdout=sp.PIPE, stderr=sp.STDOUT
    )
    lines = [
        line.strip() for line in proc.stdout.splitlines() if line.strip() != b''
    ]
...
def _check_git_repos(path, file_mode) -> None:
    ...
    if changes:
        logger.warning(
            f'{path} [{current_branch.decode()}]: {"|".join(changes)}'
        )
